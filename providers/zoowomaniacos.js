...
var O = "6a44e47dc85b8f4224962d0f4be13276", w = "https://proyectox.yoyatengoabuela.com", $ = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", q = { "User-Agent": $, Accept: "application/json, text/javascript, */*", Connection: "keep-alive", Referer: w + "/", Origin: w, "X-Requested-With": "XMLHttpRequest" };
function C(t, e) {
  return d(this, null, function* () {
    let [o, n, i] = yield Promise.all(["es-ES", "es-MX", "en-US"].map((l) => fetch(`https://api.themoviedb.org/3/${e}/${t}?api_key=${O}&language=${l}`, { headers: { "User-Agent": $ } }).then((u) => u.json()).catch(() => null))), r = o && !/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/.test(o.title || o.name || "") ? o : n || i;
    if (!r)
      return null;
